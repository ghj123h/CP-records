...
bool check(const vector<vector<int>> &ans, int n, int k) {
    ll ch = 0;
    for (int i = 0; i < n; ++i) {
        ll sum = 0;
        for (int j = 0; j < k; ++j) {
    ...
    for (int i = 1; i < n; ++i) {
        for (int j = 0; j < i; ++j) if (ans[i] == ans[j]) return false;
    }
    return true;
}
...
void solve() {
    ...
    if (k % 2 == 0) {
        ...
        do {
            ...
            next_permutation(L.begin(), L.end()); prev_permutation(R.begin(), R.end());
